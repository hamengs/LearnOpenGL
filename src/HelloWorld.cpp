...
void Drawsquare(const std::vector<float>& vertices, Shader& shader, unsigned int texture, unsigned int normalTexture, unsigned int depthMap){
    //-------------加载平面-------------------
...
void DrawCube(const std::vector<float>& vertices, Shader& shader){
    //-------------加载平面-------------------
...
int main(){
    ...
    GLfloat f2 = 1.0f/(deltaUV3.x*deltaUV2.y-deltaUV2.x*deltaUV3.y);
    glm::vec3 tangent2;
    tangent2.x = f2 * (edge3.x*deltaUV2.y-edge2.x*deltaUV3.y);
    tangent2.y = f2 * (deltaUV2.y * edge3.y - deltaUV3.y * edge2.y);
    tangent2.z = f2 * (deltaUV2.y * edge3.z - deltaUV3.y * edge2.z);
    tangent2 = glm::normalize(tangent2);

    glm::vec3 bitangent2;
    bitangent2.x = f2 * (-deltaUV2.x * edge3.x + deltaUV3.x * edge2.x);
    bitangent2.y = f2 * (-deltaUV2.x * edge3.y + deltaUV3.x * edge2.y);
    bitangent2.z = f2 * (-deltaUV2.x * edge3.z + deltaUV3.x * edge2.z);
    bitangent2 = glm::normalize(bitangent2); 

            // positions          // normals        // texCoords  //tbn
    ...
    glm::vec3 pointLightPosition = glm::vec3(0.0f,2.0f,0.5f);
    //-------------加载法线贴图--------------
    unsigned int brickNormalMap = TextureFromFile(std::string("bricks2_normal.jpg").c_str(),resourcePath("src/texture/bricks2"));
    //-------------加载纹理贴图--------------
    unsigned int brickTexture = TextureFromFile(std::string("bricks2.jpg").c_str(),resourcePath("src/texture/bricks2"));
    unsigned int brickDispMap = TextureFromFile(std::string("bricks2_disp.jpg").c_str(),resourcePath("src/texture/bricks2"));
    ...
    glDepthFunc(GL_LESS);
    glm::vec3 brickRotation(0.0f, 0.0f, 0.0f);
    ...
    while(!glfwWindowShouldClose(window)){
        ...
        ImGui::DragFloat3("Brick Rotation", glm::value_ptr(brickRotation), 1.0f);
        ImGui::End();
        ...
        brickModel = glm::rotate(brickModel, glm::radians(brickRotation.x), glm::vec3(1.0f, 0.0f, 0.0f));
        brickModel = glm::rotate(brickModel, glm::radians(brickRotation.y), glm::vec3(0.0f, 1.0f, 0.0f));
        brickModel = glm::rotate(brickModel, glm::radians(brickRotation.z), glm::vec3(0.0f, 0.0f, 1.0f));
        sceneShader.setMat4("model", brickModel);
        sceneShader.setVec3("viewPos",camera.Position);
        sceneShader.setVec3("lightPos",pointLightPosition);
        ...
        Drawsquare(vertices,sceneShader,brickTexture,brickNormalMap,brickDispMap);
        lightShader.use();
        ...
        DrawCube(cubeVertices,lightShader);
        ImGui::Render();
